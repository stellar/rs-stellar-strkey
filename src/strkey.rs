        ...
        fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
            ...
            impl<'de> Visitor<'de> for StrkeyVisitor {
                ...
                fn visit_map<M: MapAccess<'de>>(self, mut map: M) -> Result<Self::Value, M::Error> {
                    // Deserialize the variant key as an owned `String` rather than a
                    // borrowed `&str`. Non-borrowing deserializers (e.g.
                    // `serde_json::from_value` and `from_reader`, used when reading
                    // piped stdin) cannot hand out a borrowed key and would otherwise
                    // fail with "invalid type: string ..., expected a borrowed string".
