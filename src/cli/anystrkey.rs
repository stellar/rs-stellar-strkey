...
use crate::{ed25519, DecodeError, Strkey, Unredacted, UnredactedDecoded};
...
impl Serialize for UnredactedDecoded<&AnyStrkey> {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match self.0 {
            AnyStrkey::Strkey(k) => UnredactedDecoded(k).serialize(serializer),
            AnyStrkey::PrivateKey(k) => {
                let mut map = serializer.serialize_map(Some(1))?;
                map.serialize_entry("private_key_ed25519", &UnredactedDecoded(k))?;
                map.end()
            }
        }
    }
...
impl<'de> Deserialize<'de> for UnredactedDecoded<AnyStrkey> {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        struct V;

        impl<'de> Visitor<'de> for V {
            type Value = UnredactedDecoded<AnyStrkey>;
            ...
            fn visit_map<M: MapAccess<'de>>(self, mut map: M) -> Result<Self::Value, M::Error> {
                ...
                let strkey = if key == "private_key_ed25519" {
                    let UnredactedDecoded(inner) =
                        map.next_value::<UnredactedDecoded<ed25519::PrivateKey>>()?;
                    ...
                    let UnredactedDecoded(inner) =
                        serde_json::from_value::<UnredactedDecoded<Strkey>>(
