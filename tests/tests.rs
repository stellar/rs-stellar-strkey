...
fn make_payload(slice: &[u8]) -> ed25519::InnerPayloadBuf {
    heapless::Vec::from_slice(slice).unwrap()
}
...
fn test_signed_payload_ed25519_payload_sizes() {
    for payload_size in 1..=64 {
        let mut payload = ed25519::InnerPayloadBuf::new();
        (0..payload_size).for_each(|i| {
            payload.push(i as u8).unwrap();
        });
